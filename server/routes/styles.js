...
router.post('/', authMiddleware, adminOnly, (req, res) => {
  const { name, description, prompt_prefix, prompt_suffix, color, icon, scene_pattern, aspect_ratio } = req.body;
  if (!name) return res.status(400).json({ error: 'Name required' });

  const db = getDb();
  const id = uuidv4();
  const patternStr = Array.isArray(scene_pattern) ? JSON.stringify(scene_pattern) : (scene_pattern || '["image"]');
  const ar = aspect_ratio || 'IMAGE_ASPECT_RATIO_LANDSCAPE';
  db.prepare(`
    INSERT INTO styles (id, name, description, prompt_prefix, prompt_suffix, color, icon, scene_pattern, aspect_ratio, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, name, description || '', prompt_prefix || '', prompt_suffix || '', color || '#6366F1', icon || '🎬', patternStr, ar, req.user.id);
...
router.put('/:id', authMiddleware, adminOnly, (req, res) => {
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(name, description, prompt_prefix, prompt_suffix, color, icon, patternStr, aspect_ratio || null, req.params.id);
